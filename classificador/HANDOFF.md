...
- `vitaminas.py` (depois de dima.py e farma_marcas.py) → `farmaVit`; no app `compileVit`/`vitDe` (passo logo depois do
  override da CMED no `runAll`), `vitTok`/`vitDec` no complemento, `semCodLab` na marca.
